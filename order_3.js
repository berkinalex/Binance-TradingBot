...
function orden3() {
	...
	precio_orden_3 = document.getElementById("precio_orden_3").value;
	stop_orden_3 = document.getElementById("stop_orden_3").value;
	limite_orden_3 = document.getElementById("limite_orden_3").value;
	cantidad_orden_3 = document.getElementById("cantidad_orden_3").value;
	if (document.getElementById("sa_3").value == "ninguno") {
		alert("¡ No has seleccionado ningún tipo de orden !");
	} else if (document.getElementById("sa_3").value == "comprar_limit_order") {
		binance.buy(cripto, cantidad_orden_3, precio_orden_3, {
			type: "LIMIT"
		}, (error, response) => {
			if (error) {
				alert("Error al realizar la orden #3, comprueba que dispones de saldo suficiente para realizar la orden (pruebe a reducir unas monedas).");
			} else {
					...
					binance.orderStatus(cripto, orderid, function (error, json) {
						if (error) {
							...
							if (estado === "FILLED") {
								...
								document.getElementById("orden_3").style.color = "#ff0000";
								$('#orden_3').html("Cancelada");
								clearInterval(interval_3);
								if (localStorage.getItem("Alarmas") == "Si") {
									alarma_1.play();
									alarma_1.onended = function () {
	...
	} else if (document.getElementById("sa_3").value == "comprar_orden_de_mercado") {
		binance.marketBuy(cripto, cantidad_orden_3, {
			type: "MARKET"
		}, (error, response) => {
			if (error) {
				alert("Error al realizar la orden #3, comprueba que dispones de saldo suficiente para realizar la orden (pruebe a reducir unas monedas).");
	...
	} else if (document.getElementById("sa_3").value == "comprar_stop_limit") {
		binance.buy(cripto, cantidad_orden_3, precio_orden_3, {
			stopPrice: stop_orden_3,
	...
	} else if (document.getElementById("sa_3").value == "comprar_stop_limit2") {
		binance.buy(cripto, cantidad_orden_3, precio_orden_3, {
			stopPrice: stop_orden_3,
	...
	} else if (document.getElementById("sa_3").value == "comprar_oco") {
		binance.buy(cripto, cantidad_orden_3, precio_orden_3, {
			stopLimitPrice: limite_orden_3,
			stopPrice: stop_orden_3,
			type: "OCO"
		}, (error, response) => {
			if (error) {
	...
	} else if (document.getElementById("sa_3").value == "vender_limit_order") {
		binance.sell(cripto, cantidad_orden_3, precio_orden_3, {
			type: "LIMIT"
		}, (error, response) => {
			if (error) {
				alert("Error al realizar la orden #3, comprueba que dispones de saldo suficiente para realizar la orden (pruebe a reducir unas monedas).");
	...
	} else if (document.getElementById("sa_3").value == "vender_orden_de_mercado") {
		binance.marketSell(cripto, cantidad_orden_3, {
			type: "MARKET"
		}, (error, response) => {
			if (error) {
				alert("Error al realizar la orden #3, comprueba que dispones de saldo suficiente para realizar la orden (pruebe a reducir unas monedas).");
	...
	} else if (document.getElementById("sa_3").value == "vender_stop_limit") {
		binance.sell(cripto, cantidad_orden_3, precio_orden_3, {
			stopPrice: stop_orden_3,
			type: "STOP_LOSS_LIMIT"
		}, (error, response) => {
	...
	} else if (document.getElementById("sa_3").value == "vender_stop_limit2") {
		binance.sell(cripto, cantidad_orden_3, precio_orden_3, {
			stopPrice: stop_orden_3,
			type: "TAKE_PROFIT_LIMIT"
		}, (error, response) => {
	...
	} else if (document.getElementById("sa_3").value == "vender_oco") {
		binance.sell(cripto, cantidad_orden_3, precio_orden_3, {
			stopLimitPrice: limite_orden_3,
			stopPrice: stop_orden_3,
			type: "OCO"
